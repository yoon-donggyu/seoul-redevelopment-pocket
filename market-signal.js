...
(() => {
  ...
  async function load(){
    ...
    try{
      ...
      if(strong)strong.innerHTML=`서울 아파트 <b>${fmt(x.value,1)}</b> <em class="market-change ${chg>0?'up':chg<0?'down':''}">${Number.isFinite(chg)?(chg>0?'+':'')+fmt(chg,2)+'%':'-'}</em>`;
      if(small)small.textContent=`${x.label||x.period||''} · 전월 대비 · 한국부동산원 공식 월간 매매가격지수`;
