...
(() => {
  ...
  const moneyEok=n=>Number.isFinite(Number(n))?(Number(n)/10000).toFixed(Number(n)%10000?1:0)+'억':'미확인';
  ...
  function ensure(){
    ...
    sec.innerHTML=`<div class="section-head"><h2>서울 외곽 내집마련 보드</h2><small>초보자용 비교</small></div>
      ...
      <b>③ 조건</b><div class="homebuying-tools hb-quick"><button class="hb-chip on" data-quick="전체">전체</button><button class="hb-chip" data-quick="낮음">가격부담 낮음</button><button class="hb-chip" data-quick="하락">최근 하락</button><button class="hb-chip" data-quick="재개발">재개발 있음</button></div></div>
      ...
      <details class="hb-help"><summary>59㎡형·84㎡형과 신뢰도는 어떻게 보나요?</summary><div><b>59㎡형</b>은 전용 55~65㎡, <b>84㎡형</b>은 전용 80~90㎡ 거래를 묶은 참고 비교입니다. <b>신뢰 높음</b>은 최신 월 거래 20건 이상, <b>참고</b>는 5~19건, <b>표본 부족</b>은 4건 이하입니다. 예산 필터는 선택한 평형의 <b>월 중간 거래가</b> 기준입니다.</div></details>`;
  ...
  function currentRows(){
    ...
    return (DATA.rows||[]).map(x=>({...x,view:x.segments?.[segment]||null})).filter(x=>{
      ...
      if(budget&&(!Number.isFinite(Number(s.latestMedianAmount))||Number(s.latestMedianAmount)>budget))return false;
      if(quick==='낮음'&&s.priceBurden!=='낮음')return false;
      if(quick==='하락'&&!(Number(s.changePct)<0))return false;
      if(quick==='재개발'&&!(x.redevelopmentCount>0))return false;
      return true;
    }).sort((a,b)=>(a.view?.latestMedianPyeong??Infinity)-(b.view?.latestMedianPyeong??Infinity));
  }
  function render(){
    ...
    if(meta)meta.textContent=`${DATA.months?.[1]||'-'} → ${DATA.months?.[0]||'-'} · ${segmentLabel()} · ${budget?moneyEok(budget)+' 이하':'예산 전체'} · ${rows.length}개 지역`;
    root.innerHTML=rows.length?rows.map((x,i)=>{const s=x.view||{};return `<article class="hb-card">
      <div class="hb-rank">${i+1}</div>
      <div class="hb-head"><div><span>${esc(x.district)} · ${esc(segmentLabel())}</span><strong>${moneyMan(s.latestMedianPyeong)} / 평</strong></div><b class="hb-grade burden-${esc(s.priceBurden||'미확인')}">${esc(burdenText(s.priceBurden))}</b></div>
      <div class="hb-kpis"><div><span>월 중간 거래가</span><b>${moneyMan(s.latestMedianAmount)}</b></div><div><span>전월 대비</span><b class="${trendClass(s.changePct)}">${pct(s.changePct)}</b></div><div><span>최근 거래건수</span><b>${Number(s.latestCount||0).toLocaleString('ko-KR')}건</b></div><div><span>데이터 신뢰도</span><b class="conf-${confClass(s.confidence)}">${esc(s.confidence||'미확인')}</b></div></div>
      <div class="hb-status"><span class="${trendClass(s.changePct)}">${trendText(s.changePct)}</span><span>재개발 ${Number(x.redevelopmentCount||0)}곳</span><span>온비드 조회 가능</span><span>법원경매 API 대기</span></div>
      <details><summary>초보자 설명</summary><p><b>${esc(x.district)}</b>의 ${esc(segmentLabel())} 신고거래를 모아 비교한 값입니다. ${s.confidence==='표본 부족'?'<b>거래 수가 적어 변화율은 특히 조심해서 봐야 합니다.</b>':'한 달의 거래 구성에 따라 변화율이 달라질 수 있으므로 원하는 단지의 최근 실거래를 한 번 더 확인하세요.'}</p></details>
