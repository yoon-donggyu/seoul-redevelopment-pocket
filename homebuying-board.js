...
(() => {
  ...
  function render(){
    ...
    root.innerHTML=rows.length?rows.map((x,i)=>{const s=x.view||{},bad=x.status==='error';return `<article class="hb-card ${bad?'hb-error-card':''}">
      ...
      <div class="hb-kpis"><div><span>월 중간 거래가</span><b>${bad?'미확인':moneyMan(s.latestMedianAmount)}</b></div><div><span>전월 대비</span><b class="${trendClass(s.changePct)}">${bad?'미확인':pct(s.changePct)}</b></div><div><span>최근 거래건수</span><b>${bad?'미확인':Number(s.latestCount||0).toLocaleString('ko-KR')+'건'}</b></div><div><span>데이터 신뢰도</span><b class="conf-${confClass(s.confidence)}">${bad?'조회 실패':esc(s.confidence||'미확인')}</b></div></div>
      <div class="hb-status"><span class="${trendClass(s.changePct)}">${bad?'실거래 API 확인 필요':trendText(s.changePct)}</span><span>재개발 ${Number(x.redevelopmentCount||0)}곳</span><span>온비드 조회 가능</span><span>법원경매 API 대기</span></div>
      <details><summary>초보자 설명</summary><p>${bad?`국토부 API 조회 중 오류가 발생했습니다. ${esc(x.apiErrors?.[0]?.error||'잠시 후 다시 확인하세요.')}`:`<b>${esc(x.district)}</b>의 ${esc(segmentLabel())} 신고거래를 모아 비교한 값입니다. ${s.confidence==='표본 부족'?'<b>거래 수가 적어 변화율은 특히 조심해서 봐야 합니다.</b>':'한 달의 거래 구성에 따라 변화율이 달라질 수 있으므로 원하는 단지의 최근 실거래를 한 번 더 확인하세요.'}`}</p></details>
