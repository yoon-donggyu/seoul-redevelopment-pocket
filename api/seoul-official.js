...
function parseOfficial(text){
  ...
  m=match1(text,[/권리산정기준일[^0-9]{0,20}(20\d{2}[.\-년\s]+\d{1,2}[.\-월\s]+\d{1,2})/i]);if(m)out.rightsDate=m[1].replace(/년|월/g,'-').replace(/일/g,'').replace(/[.\s]+/g,'-').replace(/-+/g,'-').replace(/-$/,'');
  m=match1(text,[/정비구역\s*지정[^0-9]{0,30}(20\d{2}[.\-년\s]+\d{1,2}[.\-월\s]+\d{1,2})/i]);if(m)out.designationDate=m[1].replace(/년|월/g,'-').replace(/일/g,'').replace(/[.\s]+/g,'-').replace(/-+/g,'-').replace(/-$/,'');
  const stagePatterns=[['관리처분인가','관리처분인가'],['사업시행인가','사업시행인가'],['조합설립인가','조합설립인가'],['조합설립추진위원회승인','추진위원회 승인'],['정비구역지정','정비구역 지정']];for(const[p,label]of stagePatterns){if(text.includes(p)){out.stage=label;break}}
  return out;
}
...
export default async function handler(req,res){
  ...
  return res.status(200).json({ok:true,version:'7.4.0',project:{id:p.id,name:p.name,district:p.district,dong:p.dong},official:merged,sources:results,note:'공식 페이지에서 명시적으로 확인되는 값만 보강합니다. 찾지 못한 값은 미확인으로 유지합니다.',generatedAt:new Date().toISOString()});
}
