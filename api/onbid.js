...
async function requestCode(code,p){
  ...
  try{
    const r=await fetch(u,{signal:ctrl.signal,headers:{Accept:'application/json,application/xml,text/xml,*/*','User-Agent':'SeoulRedevelopmentPocket/7.5'}});
...
export default async function handler(req,res){
  try{
    ...
    return res.status(200).json({ok:true,version:'7.5.0',generatedAt:new Date().toISOString(),project:{id:p.id,name:p.name,district:p.district,dong:p.dong},scope:`${p.district} ${p.dong} 현재 입찰중/예정 부동산 공매`,queryMode:'official-region-filter-parallel',count:dedup.length,items:dedup,apiErrors,scanCoverage:{complete:apiErrors.length===0,maxPagesPerPropertyCode:1,scans},availableFields:[...fields].sort(),exactProjectBoundary:false,exactProjectBoundaryNote:'사업구역 Polygon 좌표 매칭 전이므로 동일 구/동 참고 공매입니다.'});
  }catch(e){res.setHeader('Cache-Control','no-store');return res.status(500).json({ok:false,error:String(e?.message||e),version:'7.5.0'})}
}
