...
async function requestPage(code,pageNo){
  ...
  try{
    const r=await fetch(u,{signal:ctrl.signal,headers:{Accept:'application/json,application/xml,text/xml,*/*','User-Agent':'SeoulRedevelopmentPocket/9.3'}});
    const text=await r.text();
    if(r.status===429){const e=new Error('HTTP 429 · 온비드 요청 제한');e.rateLimited=true;throw e}
...
export default async function handler(req,res){
  try{
    ...
    if(!code){res.setHeader('Cache-Control','no-store');return res.status(400).json({ok:false,error:'온비드 화면은 저장된 일일 데이터 파일을 사용합니다. code 파라미터가 필요합니다.',version:'9.3.0'})}
    if(!CODES.includes(code)){res.setHeader('Cache-Control','no-store');return res.status(400).json({ok:false,error:'지원하지 않는 재산유형 코드입니다.',version:'9.3.0'})}
    const d=await requestPage(code,pageNo);
    res.setHeader('Cache-Control',CACHE);
    return res.status(200).json({ok:true,version:'9.3.0',generatedAt:new Date().toISOString(),propertyCode:code,page:pageNo,rowsPerPage:ROWS,totalCount:d.totalCount,count:d.rows.length,items:d.rows.map(normalize)});
  }catch(e){
    res.setHeader('Cache-Control','no-store');
    const status=e?.rateLimited?503:500;
    return res.status(status).json({ok:false,error:String(e?.message||e),version:'9.3.0'});
  }
}
